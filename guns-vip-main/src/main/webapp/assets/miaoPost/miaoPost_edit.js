...
layui.use(['form', 'admin', 'ax','laydate','upload','formSelects'], function () {
    ...
    //获取详情信息，填充表单
    var ajax = new $ax(Feng.ctxPath + "/miaoPost/detail?postId=" + Feng.getUrlParam("postId"));
    var result = ajax.start();
    form.val('miaoPostForm', result.data);

    //表单提交事件
    form.on('submit(btnSubmit)', function (data) {
        var ajax = new $ax(Feng.ctxPath + "/miaoPost/editItem", function (data) {
            Feng.success("更新成功！");
            //传给上个页面，刷新table用
