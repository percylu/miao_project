...
layui.use(['form', 'admin', 'ax','laydate','upload','formSelects'], function () {
    ...
    //获取详情信息，填充表单
    var ajax = new $ax(Feng.ctxPath + "/miaoTopic/detail?topicId=" + Feng.getUrlParam("topicId"));
    var result = ajax.start();
    form.val('miaoTopicForm', result.data);
    $('#filePreview').attr('src', result.data.img); //图片链接（base64）
    $('#img').val(result.data.img); //图片链接（base64）
    //表单提交事件
    form.on('submit(btnSubmit)', function (data) {
        data.field.img=$('#img').val();
        var ajax = new $ax(Feng.ctxPath + "/miaoTopic/editItem", function (data) {
            Feng.success("更新成功！");
            //传给上个页面，刷新table用
